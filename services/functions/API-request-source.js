...
async function detailsHandler() {
	...
	const activatedDeposits = validators.reduce((accumulator, { validator }) => {
		...
		const activatedDuringPeriod = activation_epoch > previousReportEpoch && activation_epoch <= reportEpoch
		if (activatedDuringPeriod) {
			accumulator += 1
		}
		return accumulator
	}, 0)
	...
	const completedExits = validators.reduce((accumulator, { status, validator }) => {
		...
		const completedDuringPeriod = withdrawable_epoch >= previousReportEpoch && withdrawable_epoch < reportEpoch
		const completed = status === "withdrawal_done"
		if (completedDuringPeriod && completed) {
			accumulator += 1
		}
		return accumulator
	}, 0)
