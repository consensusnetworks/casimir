...
const [
    ...
    getStakedPoolCountSignature,
    getStakedPoolPublicKeysSignature,
    getStakedPoolStatusesSignature,
...
async function balancesHandler() {
    const stakedPoolCount = await getStakedPoolCount()
    const startIndex = BigInt(0).toString(16).padStart(64, "0")
    const endIndex = BigInt(stakedPoolCount).toString(16).padStart(64, "0")

    const stakedPoolPublicKeys = await getStakedPoolPublicKeys(startIndex, endIndex)
    const validators = await getValidators(stakedPoolPublicKeys)
...
async function detailsHandler() {
    ...
    const stakedPoolPublicKeys = await getStakedPoolPublicKeys(startIndex, endIndex)
    const stakedPoolStatuses = await getStakedPoolStatuses(startIndex, endIndex) // Not used yet
    const validators = await getValidators(stakedPoolPublicKeys)
...
async function getStakedPoolCount() {
    const request = await Functions.makeHttpRequest({
        url: ethereumUrl,
        method: "POST",
        data: {
            ...
            params: [
                {
                    to: viewsAddress,
                    data: getStakedPoolCountSignature
                },
                "latest"
            ]
        }
...
async function getStakedPoolPublicKeys(startIndex, endIndex) {
    const request = await Functions.makeHttpRequest({
        url: ethereumUrl,
        method: "POST",
        data: {
            ...
            params: [
                {
                    to: viewsAddress,
                    data: getStakedPoolPublicKeysSignature + startIndex + endIndex
                },
                "latest"
            ]
        }
    ...
    for (let i = 0; i < itemSize; i++) {
        ...
        for (let j = 0; j < chunkCount; j++) {
            itemChunk = chunks[itemIndex + 1 + j]
...
async function getStakedPoolStatuses(startIndex, endIndex) {
    const request = await Functions.makeHttpRequest({
        url: ethereumUrl,
        method: "POST",
        data: {
            ...
            params: [
                {
                    to: viewsAddress,
                    data: getStakedPoolStatusesSignature + startIndex + endIndex
                },
                "latest"
            ]
        }
