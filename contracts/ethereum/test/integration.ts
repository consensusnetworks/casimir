...
import { firstUserDepositFixture, rewardsPostSecondUserDepositFixture, secondUserDepositFixture, thirdUserDepositFixture, rewardsPostThirdUserDepositFixture, simulationFixture, firstUserPartialWithdrawalFixture, fourthUserDepositFixture, sweepPostSecondUserDepositFixture, sweepPostThirdUserDepositFixture, activeBalanceLossFixture, activeBalanceRecoveryFixture, thirdUserWithdrawalCompletedFixture } from './fixtures/shared'
...
describe('Integration', async function () {
    ...
    it('Third user full withdrawal is completed on exit report', async function () {
        const { manager, firstUser, secondUser, thirdUser, fourthUser } = await loadFixture(thirdUserWithdrawalCompletedFixture)
        const stake = await manager.getTotalStake()
        const firstStake = await manager.getUserStake(firstUser.address)
        const secondStake = await manager.getUserStake(secondUser.address)
        const thirdStake = await manager.getUserStake(thirdUser.address)
