...
interface ISSVNetwork is ISSVNetworkCore {
    ...
    function validatorPKs(
        bytes32 validatorId
    ) external view returns (address owner, bool active);

    function clusters(
        bytes32 clusterId
    ) external view returns (bytes32 clusterData);
    ...
    function operatorFeeChangeRequests(
        ...
        external
        view
        returns (uint64 fee, uint64 approvalBeginTime, uint64 approvalEndTime);

    function operatorsWhitelist(
        uint64 operatorId
    ...
    function dao()
        external
        view
        returns (uint32 validatorCount, uint64 balance, uint64 block);
