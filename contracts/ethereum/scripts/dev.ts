...
void async function () {
      ...
      if (block - blocksPerReport >= lastReportBlock) {
        ...
        if (pendingPoolIds.length + stakedPoolIds.length) {
          ...
          const nextBeaconBalance = round(
            parseFloat(ethers.utils.formatEther(latestBeaconBalance))
              + activatedBalance + rewardBalance - sweptRewardBalance - sweptExitedBalance, 10)
          ...
          if (remaining) {
            for (const poolId of stakedPoolIds) {
              ...
              if (
                poolDetails.status === PoolStatus.EXITING_FORCED 
                || poolDetails.status === PoolStatus.EXITING_REQUESTED
              ) {
                remaining--
                const poolAddress = await manager.getPoolAddress(poolId)
                const currentBalance = await ethers.provider.getBalance(poolAddress)
  ...
  setTimeout(async () => {
    ...
    const depositStake = await manager
      .connect(firstUser)
